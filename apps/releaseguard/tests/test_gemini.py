...
async def test_gemini_judge_missing_key():
    ...
    with patch("app.skills.gemini_judge.settings") as mock_settings:
        ...
        evidence = [
            EvidenceItem(
                category="api_health", status="success", message="OK", risk_score=0
            )
        ]
...
@pytest.mark.asyncio
async def test_gemini_approval_is_evidence_only_not_traffic_authority():
    response = MagicMock()
    response.text = json.dumps(
        {
            ...
            "unsafe_actions": ["Treating this judgement as traffic authority"],
    ...
    with (
        patch("app.skills.gemini_judge.settings") as mock_settings,
        patch("app.skills.gemini_judge.genai.Client", return_value=client),
    ):
        mock_settings.GEMINI_API_KEY = "test-only"
        mock_settings.GEMINI_MODEL = "gemini-2.5-flash"
        judge = GeminiJudge()
        await judge.judge(
            EvaluationRequest(
    ...
    assert "does not authorize merge or production traffic" in (
        config.system_instruction
    )
    assert "additional human review" in config.system_instruction
...
async def test_orchestrator_gemini_cannot_override_block():
    ...
    assert any(
        "Checkout page failed or checkout button was missing/invisible" in rule
...
async def test_orchestrator_gemini_blocks_when_preliminary_approves():
    ...
    mock_api_probe.evaluate.return_value = [
        EvidenceItem(
            category="api_checkout", status="success", message="Visible", risk_score=0
        )
    ]
    mock_secret_scan = AsyncMock()
    mock_secret_scan.evaluate.return_value = []
    ...
    assert any(
        "Gemini AI recommended BLOCK due to high risk." in rule
        for rule in decision.triggered_rules
    )
